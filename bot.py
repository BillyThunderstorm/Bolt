...
def process_recording(
    ...
    notify("Step 3/6 — Generating titles…", level="info",
           reason=("AI titles are enabled; Bolt will use cached LLM captions with template fallback."
                   if use_ai_titles else
                   "Using local templates to generate TikTok titles. "
    ...
    try:
        ...
        if retrieved_count:
            direction = memory_influence.get("net_direction", "neutral") if isinstance(memory_influence, dict) else "neutral"
            memory_reason += (
