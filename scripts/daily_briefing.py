...
def _format_unified_decision_hit(evt: dict) -> dict | None:
    ...
    if already_handled:
        score = 0.4
    elif result in _ACTIONABLE_RESULTS or feedback:
        score = 0.9
    ...
    return {
        ...
        "needs_follow_up": bool(
            not already_handled
            and (result in _ACTIONABLE_RESULTS or feedback)
        ),
    }
...
def _load_previous_briefing_comments() -> list:
    ...
    for raw in body.splitlines():
        ...
        if not line:
            continue
        # Skip the generated how-to blurb
...
def _retrieve_briefing_memory(query: str = "", limit: int = 5) -> list:
    ...
    # Source 2b: comments Billy wrote on the last briefing
    try:
        hits.extend(_load_previous_briefing_comments())
    except Exception:
        pass
...
def _memory_to_action_items(hits: list) -> list:
    ...
    for h in hits[:hit_cap]:
        ...
        if not text:
            continue
        if kind == "constraint":
            # Profile reminders stay in Memory Notes, not the to-do list.
            continue
...
def generate_briefing():
    ...
    text += (
        "\n---\n\n"
        "## Comments\n\n"
        "Write below this line. The next briefing will surface what you wrote "
        "as a note. To change a hard rule (C1–C7), say so in chat or edit "
        "`Data/memory/user_profile.json` — comments here do not rewrite the profile.\n\n"
    )
