...
# This looks for .env in the same folder as this file
...
def get_follower_count():
    ...
    try:
        ...
        response_data = follower_response.json()
        
        # Debug: Print what we actually got back
        print(f"DEBUG - Follower API Response: {response_data}")
        
        # Check if data array is empty before accessing [0]
        if not response_data.get("data"):
            print("WARNING: Follower data is empty. Check your token permissions.")
            return None
        
        follower_count = response_data["data"][0]["total"]
