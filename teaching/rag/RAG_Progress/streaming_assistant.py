...
def get_streaming_tip(situation):
    ...
    class StreamTip(dspy.Signature):
        ...
        tip = dspy.OutputField(desc="One practical, actionable tip to improve the situation")
...
def get_streaming_setup_advice(current_setup, budget, goals, platform="Twitch"):
    ...
    class StreamAdvice(dspy.Signature):
        ...
        current_setup = dspy.InputField(desc="Current equipment/software (can be 'none')")
        budget = dspy.InputField(desc="Budget for upgrades (e.g., '$0-100', '$100-500', '$500+')")
        goals = dspy.InputField(desc="Streaming goals (e.g., 'grow audience', 'improve quality')")
        platform = dspy.InputField(desc="Target platform (Twitch, YouTube, etc.)")
        
        priority_upgrades = dspy.OutputField(desc="Top 1-3 most impactful upgrades within budget")
        settings_optimization = dspy.OutputField(desc="Key OBS/streamlabs settings to adjust")
    ...
    while True:
        try:
            ...
            if choice.lower() in ['quit', 'exit', '4']:
                ...
                print("Examples: 'My audio sounds echoey', 'I'm not getting chat interaction', 'Webcam too dark'")
                ...
                current = input("Current setup (e.g., 'laptop with built-in mic', 'none'): ").strip()
                if not current:
                    current = "none"
                    
                budget = input("Budget for upgrades (e.g., '$0-100', '$100-500', '$500+'): ").strip()
                if not budget:
                    budget = "$0-100"
                    
                goals = input("Your goals (e.g., 'start streaming', 'improve quality', 'grow to affiliate'): ").strip()
                if not goals:
                    goals = "start streaming and have fun"
                    
                platform = input("Platform (Twitch, YouTube, Kick, etc.) [Twitch]: ").strip() or "Twitch"
                ...
                print("  • Current: 'Blue Yeti mic, laptop webcam', Budget: '$100-500', Goals: 'improve quality'")
                print("  • Current: 'Desktop PC, no mic', Budget: '$500+', Goals: 'go full-time'")
                print()
