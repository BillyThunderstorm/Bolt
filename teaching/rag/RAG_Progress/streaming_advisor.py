...
def test_streaming_advisor():
    ...
    class StreamAdvice(dspy.Signature):
        ...
        current_setup = dspy.InputField(desc="What equipment/software the user currently has (can be 'none' for beginner)")
        budget = dspy.InputField(desc="Budget for upgrades (e.g., '$0-100', '$100-500', '$500+', or 'no budget')")
        goals = dspy.InputField(desc="Streaming goals (e.g., 'grow audience', 'improve quality', 'start streaming', 'specific game/content')")
        platform = dspy.InputField(desc="Target platform (Twitch, YouTube, Kick, etc.)")
        
        # Output recommendations
        priority_upgrades = dspy.OutputField(desc="Top 1-3 most impactful upgrades or changes within budget")
        settings_optimization = dspy.OutputField(desc="Key OBS/streamlabs settings to adjust for better quality/performance")
        engagement_tips = dspy.OutputField(desc="Specific tips to increase viewer engagement and retention")
        quick_wins = dspy.OutputField(desc="Easy, immediate improvements that cost little or nothing")
        avoidance = dspy.OutputField(desc="Common mistakes to avoid for someone at this level")
