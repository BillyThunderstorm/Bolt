...
def test_content_planner():
    ...
    class ContentPlan(dspy.Signature):
        ...
        niche = dspy.InputField(
            desc="Content niche: tech reviews, gaming, skincare, streaming, or AI development"
        )
        topic = dspy.InputField(desc="Specific topic or product to cover")
        audience = dspy.InputField(
            desc="Target audience: beginners, intermediate, experts, or general"
        )
        format_type = dspy.InputField(
            desc="Content format: YouTube video, TikTok, blog post, livestream, or Twitter thread"
        )
        ...
        key_points = dspy.OutputField(
            desc="3-5 main points to cover, each with a brief description"
        )
        structure = dspy.OutputField(
            desc="Suggested structure/format with timing or sections"
        )
        engagement_tips = dspy.OutputField(
            desc="Specific tips to increase audience engagement"
        )
        call_to_action = dspy.OutputField(
            desc="Clear call-to-action for viewers/readers"
        )
