...
def _split_markdown_sections(path: Path, root: Path) -> List[MemoryEntry]:
    ...
    def flush() -> None:
        ...
        if not body:
            return
        # `path` may live under MEMORY_DIR or alongside it (e.g. content/).
        # Use whichever ancestor we have, falling back to no directory tags.
        try:
            dir_parts = path.relative_to(MEMORY_DIR).parts[:-1]
        except ValueError:
            try:
                dir_parts = path.relative_to(root).parts[:-1]
            except ValueError:
                dir_parts = ()
        tags = ["markdown", *dir_parts]
        sections.append(
            MemoryEntry(
