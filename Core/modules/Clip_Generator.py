...
def _cut_clip_ffmpeg(source: str, output: str, start: float, duration: float) -> tuple:
    """
    ...
    cmd = [
        ...
        "-y",
        "-i",
        source,  # input FIRST — then seek (slower but audio stays in sync)
        "-ss",
        str(start),  # accurate seek after input avoids audio dropout
        "-t",
        str(duration),
        "-c:v",
        "copy",  # copy video stream (fast, no quality loss)
        "-c:a",
        "aac",  # re-encode audio — fixes sync issues with Opus/OBS recordings
        "-b:a",
